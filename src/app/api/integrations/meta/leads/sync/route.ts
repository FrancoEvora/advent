...
export async function POST(req:NextRequest){
 try{
  ...
  const config=await getMetaGraphConfig(organizationId,pageId);const proof=createHmac("sha256",config.appSecret).update(config.accessToken).digest("hex");
