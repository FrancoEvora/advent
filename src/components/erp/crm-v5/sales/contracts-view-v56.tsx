...
export function ContractsViewV56({data,sales,reload}:{data:ErpData;sales:SalesData;reload:()=>Promise<void>}){
 ...
 async function share(contract:SalesContract,channel:"email"|"whatsapp"|"link"){
  ...
  if(channel==="link"){await logContractSend(data,contract,"link","",url);await navigator.clipboard.writeText(url);alert("Contrato assinado pela empresa e link de assinatura copiado.")}
  else if(channel==="whatsapp"){const phone=(proposal.customer_phone||"").replace(/\D/g,"");await logContractSend(data,contract,"whatsapp",phone,url);window.open(`https://wa.me/${phone}?text=${encodeURIComponent(`Olá, ${proposal.customer_name}. Acesse e assine o contrato ${contract.contract_number}: ${url}`)}`,"_blank")}
  else{await logContractSend(data,contract,"email",proposal.customer_email||"",url);location.href=`mailto:${proposal.customer_email||""}?subject=${encodeURIComponent(`Contrato ${contract.contract_number} — Évora Urbanismo`)}&body=${encodeURIComponent(`Olá, ${proposal.customer_name}.\n\nAcesse, confira e assine eletronicamente o contrato: ${url}`)}`}
  await reload();
 }

 async function cancelUnsigned(contract:SalesContract){
  const reason=prompt("Informe o motivo do cancelamento do contrato ainda não assinado pelo comprador:");if(!reason)return;
 ...
 return <div className="crm5-stack">
  ...
  <section className="crm5-section-header"><div><small>ASSINATURA ELETRÔNICA E RECEBÍVEIS</small><h2>Contratos</h2><p>Envio, assinatura auditável, cancelamento prévio e integração automática ao financeiro.</p></div></section>
  <section className="contract-list">{sales.contracts.map(contract=>{const proposal=sales.proposals.find(item=>item.id===contract.proposal_id),unit=sales.units.find(item=>item.id===contract.unit_id),emitted=sales.installments.filter(item=>item.proposal_id===contract.proposal_id&&item.financial_entry_id).length,companySigned=!!(contract as any).company_signed_at,customerSigned=!!(contract as any).customer_signed_at,signatureStatus=(contract as any).signature_status||"pendente_empresa",canCancelUnsigned=canDecide&&!customerSigned&&contract.status!=="cancelado";return <article key={contract.id}><header><div><small>{contract.contract_number}</small><h3>{proposal?.customer_name||"Cliente"}</h3><span>{data.projects.find(item=>item.id===contract.project_id)?.name} · {unit?.unit_code}</span></div><i data-status={contract.status}>{statusLabel[contract.status]||contract.status}</i></header><div className="contract-stats"><span><small>Valor</small>{brl.format(Number(proposal?.sale_price||0))}</span><span><small>Empresa</small>{companySigned?"Assinado":"Pendente"}</span><span><small>Comprador</small>{customerSigned?"Assinado":"Pendente"}</span><span><small>Recebíveis</small>{emitted}</span></div><div className="signature-flow-status" data-status={signatureStatus}><strong>{signatureStatus==="concluida"?"Assinatura bilateral concluída":signatureStatus==="pendente_cliente"?"Aguardando assinatura do comprador":"Aguardando envio e assinatura da empresa"}</strong>{(contract as any).document_hash&&<code>{(contract as any).document_hash}</code>}</div>{contract.cancellation_reason&&<div className="feedback error">Cancelado: {contract.cancellation_reason}</div>}<footer><button onClick={()=>printContract(contract)}>Gerar contrato / PDF</button>{contract.status!=="cancelado"&&<><button onClick={()=>share(contract,"link")}>Assinar pela empresa e copiar link</button><button onClick={()=>share(contract,"whatsapp")}>WhatsApp</button><button onClick={()=>share(contract,"email")}>E-mail</button></>}{canCancelUnsigned&&<button className="danger-button" onClick={()=>cancelUnsigned(contract)}>Cancelar antes da assinatura</button>}{customerSigned&&contract.status==="assinado"&&canAdmin(data.membership.role)&&<button className="danger-button" onClick={()=>setCancel(contract)}>Distrato / cancelamento jurídico</button>}</footer></article>})}{!sales.contracts.length&&<div className="crm5-empty"><strong>Nenhum contrato gerado</strong><p>O contrato será criado automaticamente quando o cliente aceitar a proposta.</p></div>}</section>
