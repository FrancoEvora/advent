...
do $test$
declare
  org uuid;actor uuid;ct uuid;thread uuid;op uuid;ambiguous_thread uuid;alias_phone text;result jsonb;again jsonb;args jsonb;before_crm bigint;before_jobs bigint;n bigint;refused boolean;
...
begin
  ...
  v_phone:='553498'||lpad(floor(random()*10000000)::bigint::text,7,'0');
  alias_phone:=left(v_phone,4)||substr(v_phone,6);
  if crm_private.whatsapp_phone_variants('553499294123')<>array['553499294123','5534999294123']
    or crm_private.whatsapp_phone_variants('5534999294123')<>array['5534999294123','553499294123']
    or crm_private.whatsapp_phone_variants('553432201234')<>array['553432201234']
    or crm_private.whatsapp_phone_variants('155599294123')<>array['155599294123'] then
    raise exception 'Mobile aliases changed landlines or other countries';
  end if;
  ...
  result=public.arisa_whatsapp_webhook(org,receiver,jsonb_build_object('messages',jsonb_build_array(jsonb_build_object('provider_message_id',inbound_id,'from_phone',alias_phone,'profile_name','QA fornecedor','content','Podemos alinhar a drenagem às 10h.','message_type','text','occurred_at',now()))));
  ...
  result=public.arisa_whatsapp_webhook(org,receiver,jsonb_build_object('messages',jsonb_build_array(jsonb_build_object('provider_message_id',inbound_id,'from_phone',alias_phone,'content','Duplicate','occurred_at',now()))));
  ...
  -- Both forms must reuse the existing thread, including a reverse-format delivery.
  result=public.arisa_whatsapp_webhook(org,receiver,jsonb_build_object('messages',jsonb_build_array(jsonb_build_object('provider_message_id',inbound_id,'from_phone',v_phone,'content','Duplicate from full number','occurred_at',now()))));
  if (select count(*) from public.arisa_whatsapp_messages where organization_id=org and provider_message_id=inbound_id)<>1 then raise exception 'Alias duplicated inbound';end if;
  if (select metadata->>'provider_from_phone' from public.arisa_whatsapp_messages where organization_id=org and provider_message_id=inbound_id)<>alias_phone then raise exception 'Provider identity not retained';end if;
  insert into public.arisa_whatsapp_threads(organization_id,phone_number_id,phone) values(org,receiver,alias_phone) returning id into ambiguous_thread;
  result=public.arisa_whatsapp_webhook(org,receiver,jsonb_build_object('messages',jsonb_build_array(jsonb_build_object('provider_message_id','wamid.fixture.ambiguous.'||inbound_id,'from_phone',alias_phone,'content','Do not guess','occurred_at',now()))));
  if jsonb_array_length(result->'handled_message_ids')<>1 or exists(select 1 from public.arisa_whatsapp_messages where organization_id=org and provider_message_id='wamid.fixture.ambiguous.'||inbound_id) then raise exception 'Ambiguous administrative thread routed or persisted';end if;
  delete from public.arisa_whatsapp_threads where id=ambiguous_thread;
  ...
  result=public.arisa_whatsapp_webhook(org,receiver,jsonb_build_object('statuses',jsonb_build_array(jsonb_build_object('provider_message_id',status_id,'status','read','operation_id',op,'recipient_phone',alias_phone,'occurred_at',now()))));
  ...
  perform public.arisa_whatsapp_webhook(org,receiver,jsonb_build_object('messages',jsonb_build_array(jsonb_build_object('provider_message_id','wamid.fixture.future.'||gen_random_uuid()::text,'from_phone',alias_phone,'content','Future spoof','occurred_at',now()+interval '1 year'))));
  ...
  raise notice 'PASS: Brazilian ninth-digit inbound/status mapping, ambiguity, channel isolation, paused inbound, idempotency, canonical recipient, window/templates, unknown callback recovery, monotonic status, blocked contacts and service-only access';
end $test$;
rollback;
