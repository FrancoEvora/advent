...
export default function ArisaMailPanel({ organizationId, userId, section = "email" }: { organizationId: string; userId: string; section?: "email" | "agenda" }) {
  ...
  useEffect(() => {
    ...
    return () => { alive = false; };
  }, [organizationId, page, revision, section]);
  ...
  return <section><div className={styles.card}><div className={styles.meta}>GOOGLE WORKSPACE</div><h3>{status?.connected_email || status?.sender_email || "Conta corporativa da Arisa"}</h3><p>{!status ? "Verificando a conexão…" : status.connected ? "Conta conectada para envio e arquivamento de e-mails." : "A conta precisa ser autorizada no Google para começar a enviar e sincronizar mensagens."}</p><small>Última sincronização: {displayDate(status?.last_sync_at)}</small>{status?.sync_error && <p role="status">A sincronização requer atenção: {status.sync_error}. Os conteúdos já arquivados estão preservados.</p>}<div className={styles.row}><button className={styles.primary} disabled={busy || !status?.configured} onClick={() => void connect()}>{status?.connected ? "Renovar conexão Google" : "Conectar Google Workspace"}</button>{status?.connected && <><button disabled={busy} onClick={async () => { const result = await perform("sync"); if (result) setNotice(result.busy ? "Já existe uma sincronização em andamento." : `${result.archived || 0} mensagens verificadas. A sincronização continuará automaticamente.`); }}>Sincronizar agora</button><button disabled={busy} onClick={async () => { if (await perform("disconnect")) setNotice("Conta desconectada. O arquivo foi preservado."); }}>Desconectar conta</button></>}</div></div>
    ...
    {section === "agenda" && <ArisaCalendarPanel organizationId={organizationId} connected={status?.connected === true} authorized={status?.calendar_authorized === true} busy={busy || !status?.configured} onConnect={() => void connect("calendar")} />}
  </section>;
}
