...
export type ViewId = "dashboard" | "financeiro" | "caixa" | "aprovacoes" | "centros" | "cadastros" | "projetos" | "usuarios" | "relatorios" | "auditoria" | "configuracoes";
...
export interface FinancialEntry {
  ...
  category: string; category_id: string | null; cost_center_id: string | null; bank_account_id: string | null;
...
export interface ApprovalRequest { id: string; organization_id: string; entry_id: string; requested_by: string; assigned_to: string | null; status: "pendente" | "aprovado" | "rejeitado" | "cancelado"; comment: string | null; decided_at: string | null; created_at: string; }
...
export interface Settings { organization_id: string; approval_threshold: number; require_approval: boolean; default_due_alert_days: number; }

export interface ErpData {
  session: Session; organization: Organization; membership: Membership; profile: Profile | null;
  entries: FinancialEntry[]; costCenters: CostCenter[]; categories: Category[]; bankAccounts: BankAccount[];
