...
function publicAgentExperience(value: unknown): Obj {
  ...
  return {
    ...
    title: currentAgentCopy(value.title) || str(value.title) || "Atendimento Futura Casa",
...
function publicSessionContext(value: unknown): Obj {
  ...
  return {
    ...
    experience: {
      ...
      name: str(experience.name)?.slice(0, 180) || "Solaris Residencial Resort",
      agentName: publicAgentName(experience.agentName).slice(0, 80),
      title: str(experience.title)?.slice(0, 240) || "Atendimento Futura Casa",
...
async function createSimulationPdf(
  ...
  return {
    ...
    badge: "Preparado pela Futura Casa",
...
function policyReply(ctx:Obj){const policy=obj(ctx.policy)?ctx.policy:null;if(!policy)return "As condições não carregaram agora. Posso tentar novamente ou pedir a confirmação ao time comercial.";const description=str(policy.description)||"Tenho as condições vigentes prontas para simular.";const parameters=obj(policy.parameters)?policy.parameters:{};const disclaimer=str(parameters.disclaimer)||"As condições dependem da disponibilidade do lote e da análise cadastral e validação comercial da Évora Urbanismo.";return `${description} ${disclaimer}`;}
...
function currentEnterpriseProject(context: Obj, enterprise: Obj): Obj | null {
  const projects = Array.isArray(enterprise.projects) ? enterprise.projects.filter(obj) : [];
  const currentId = str(enterprise.currentProjectId) || str(context.projectId);
  if (currentId) {
    const exact = projects.find((project) => str(project.id) === currentId);
    if (exact) return exact;
  }
  if (projects.length === 1) return projects[0];
  const experience = obj(context.experience) ? context.experience : {};
  const experienceName = str(experience.name)?.toLocaleLowerCase("pt-BR") || "";
  return projects.find((project) => {
    const name = str(project.name)?.toLocaleLowerCase("pt-BR") || "";
    return name && experienceName && (name.includes(experienceName) || experienceName.includes(name));
  }) || null;
}

function currentEnterpriseProjectNames(context: Obj, project: Obj | null): string[] {
  const experience = obj(context.experience) ? context.experience : {};
  return [str(project?.name), str(experience.name)]
    .filter((name): name is string => Boolean(name));
}

function approvedProjectFacts(context: Obj): string[] {
  const knowledge = obj(context.knowledge) ? context.knowledge : {};
  return Array.isArray(knowledge.approvedFacts)
    ? knowledge.approvedFacts.filter((fact): fact is string => typeof fact === "string")
    : [];
}
...
if(currentProjectOverviewIntent){requestContact=false;handoffRequested=false;contact.collecting=obj(context.contactCapture)&&context.contactCapture.collecting===true;}
...
else if(currentProjectOverviewIntent){finalReply=currentProjectOverviewReply({message:userMessage,projectName:str(currentProject?.name)||str(obj(context.experience)?context.experience.name:null)||"empreendimento atual",city:str(currentProject?.city),approvedFacts:approvedProjectFacts(context),buyerIntent:profile.intent||currentProfile.intent||null,fallbackReply:finalReply});quickReplies=currentProjectOverviewQuickReplies();attachments=[];}
...
else if(action==="show_documents"){attachments=documents;finalReply=attachments.length?"Separei os materiais que tenho por aqui. Pode abrir qualquer item abaixo; se quiser, eu também explico os pontos principais.":"Ainda não tenho um material aprovado para enviar por aqui. Posso pedir o arquivo certo à equipe da Futura Casa.";quickReplies=attachments.length?["Explicar os materiais","Ver lotes disponíveis","Agendar visita"]:["Falar com a equipe"];}
...
async function handleMessageV4(
  ...
  try {
      ...
      if (serviceDecision === false) {
        ...
        if (!contact.name) {
          ...
          response = deterministicReply(
            ...
            { quickReplies: ["Autorizo o contato da Futura Casa"], contact },
          );
        }
    ...
    if (currentProjectOverviewRequested(userMessage, [str(experience.name) || ""])) {
      const enterprise = await rpc(admin, "get_public_agent_enterprise_context", {
        p_slug: slug,
      }) as Obj;
      const project = currentEnterpriseProject(context, enterprise);
      const response = deterministicReply(
        context,
        currentProjectOverviewReply({
          message: userMessage,
          projectName: str(project?.name) || str(experience.name) || "empreendimento atual",
          city: str(project?.city),
          approvedFacts: approvedProjectFacts(context),
          buyerIntent: currentProfile.intent || null,
        }),
        {
          stage: safeStage(context.stage),
          action: "none",
          unitCode: currentProfile.selected_unit_code || null,
          quickReplies: currentProjectOverviewQuickReplies(),
        },
      );
      const final = await finalizeMessage(admin, {
        slug,
      ...
      if (serviceDecision === false) {
        ...
        response = deterministicReply(
          ...
          serviceConsentPrompt("hold"),
          {
            action: "request_hold",
            unitCode: selectedUnit,
            quickReplies: ["Autorizo o contato da Futura Casa"],
            contact,
          },
        );
      } else {
