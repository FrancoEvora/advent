...
export function SettingsView({ data, crm, reload, can = () => false }: { data: ErpData; crm: CrmEnterpriseData; reload: () => Promise<void>; can?: (permission: string) => boolean }) {
  ...
  return <div className="crm5-stack">
    <CrmSectionHeader eyebrow="CONFIGURAÇÃO" title="Parâmetros e integrações" description="Toda a configuração de canais do CRM fica aqui. Meta Leads é configurado exclusivamente nesta tela." />
    ...
    <section className="crm5-integrations">{providers.map((provider) => {
      ...
      const status = provider === "meta" ? (metaActive ? "rota ativa" : crm.metaLeadRoutes.length ? "configuração pendente" : "não configurado") : current?.status || "não configurado";
      const description = provider === "meta"
        ? crm.metaLeadRoutes.length ? `${crm.metaLeadRoutes.length} rota(s) Page/Form cadastrada(s) · credenciais e sincronização abaixo` : "Credenciais, Página, Formulário, destino e distribuição são configurados abaixo"
        : current?.last_sync_at ? `Sincronizado em ${new Date(current.last_sync_at).toLocaleString("pt-BR")}` : "Credenciais externas necessárias";
      return <article key={provider}><div className="crm5-integration-icon">{providerIcon(provider)}</div><div><strong>{providerLabel(provider)}</strong><small>{description}</small></div><Status tone={provider === "meta" && metaActive ? "info" : status === "conectado" ? "success" : "neutral"}>{status}</Status><button disabled={!canManage} onClick={() => provider === "meta" ? document.getElementById("meta-lead-integration")?.scrollIntoView({ behavior: "smooth", block: "start" }) : save(provider, current?.status === "conectado" ? "desconectado" : "configuracao_pendente")}>{provider === "meta" ? "Configurar Meta Leads" : "Configurar"}</button></article>;
    })}</section>
