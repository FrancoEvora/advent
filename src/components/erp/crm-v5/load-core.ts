...
export async function loadCrmCore(organizationId:string) {
  ...
  const [records,actions,pipelines,stages,teams,members,alerts,assignments]=await Promise.all([
  ...
  const failed=[records,actions,pipelines,stages,teams,members,alerts,assignments].find(item=>item.error); if(failed?.error) throw failed.error;
  return {records:records.data??[],actions:actions.data??[],pipelines:pipelines.data??[],stages:stages.data??[],teams:teams.data??[],teamMembers:members.data??[],alerts:alerts.data??[],assignments:assignments.data??[]};
}
