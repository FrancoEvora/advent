alter table crm_private.arisa_whatsapp_finance_sessions add column original_requested_at timestamptz;
...
begin
 ...
 select coalesce(jsonb_agg(to_jsonb(item) order by item.created_at desc,item.id desc),'[]'::jsonb) into v_result from(
  ...
  where n.organization_id=p_organization_id and n.recipient_user_id=auth.uid() and n.metadata->>'source'='arisa_whatsapp' and (not p_unread_only or n.read_at is null)
 ...
 if private.arisa_finance_valid(t.id,j.organization_id) then
  if p_action<>'consult' then
   return jsonb_build_object('verified',true,'handled',false,'redacted_message_ids',redacted)
    || case when cached.result->>'just_verified'='true' then jsonb_build_object('just_verified',true,'request',s.original_request,'request_at',s.original_requested_at) else '{}'::jsonb end;
  end if;
 ...
 if s.locked_until>now() then
  ...
  if not coalesce(v_ready,false) then
   ...
   if s.thread_id is null then
    insert into crm_private.arisa_whatsapp_finance_sessions(thread_id,organization_id,contact_id,original_request,original_requested_at,challenge_until)
     values(t.id,j.organization_id,c.id,left(m.content,3000),m.occurred_at,now()+interval '15 minutes') returning * into s;
   elsif p_action='start' and (s.stage<>'awaiting' or s.challenge_until<=now()) then
    update crm_private.arisa_whatsapp_finance_sessions set contact_id=c.id,stage='awaiting',original_request=left(m.content,3000),original_requested_at=m.occurred_at,
   ...
   if s.stage='awaiting' and s.challenge_until<=now() then
    ...
    if v_doc is not null and v_email is not null then
     if v_doc=regexp_replace(c.document,'[^0-9]','','g') and v_email=lower(trim(c.email))
      ...
      v_result=jsonb_build_object('verified',true,'handled',false,'just_verified',true,'request',s.original_request,'request_at',s.original_requested_at);
     else
