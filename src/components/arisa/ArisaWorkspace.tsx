...
import BiaCommercialPanel from "../bia/BiaCommercialPanel";
import BiaWhatsAppPanel from "../bia/BiaWhatsAppPanel";
import ArisaMailPanel from "./ArisaMailPanel";
import ArisaWhatsAppPanel from "./ArisaWhatsAppPanel";
import ArisaWhatsAppConversations from "./ArisaWhatsAppConversations";
import { displayDate, downloadText, workspaceCall, type WorkspacePanel } from "./workspace-client";
import { assistantWorkspacePanels, workspaceLabel } from "./workspace-navigation";
...
export default function ArisaWorkspace({ organizationId, userId, initialPanel: tab, onPanelChange: setTab, onClose, assistant = "arisa" }: { assistant?: AssistantId; organizationId: string; userId: string; initialPanel: WorkspacePanel; onPanelChange: (panel: WorkspacePanel) => void; onClose: () => void }) {
  const profile = assistantProfiles[assistant];
  const dialog = useRef<HTMLDialogElement>(null);
  const [error, setError] = useState(""), [notice, setNotice] = useState(""), [busy, setBusy] = useState(false);
  const knowledgePanel = tab === "archive" || tab === "memory";
  const conversationPanel = tab === "whatsapp-conversations" || (assistant === "bia" && tab === "whatsapp");
  ...
  return <dialog ref={dialog} className={`${styles.dialog} ${conversationPanel ? styles.monitorDialog : ""}`} onCancel={onClose} aria-labelledby="arisa-workspace-title"><header className={styles.header}><div><small>{profile.name.toUpperCase()} · ÉVORA URBANISMO</small><h2 id="arisa-workspace-title">{conversationPanel ? workspaceLabel(tab, assistant) : "Canais e conhecimento"}</h2></div><button onClick={onClose} aria-label="Fechar e voltar à conversa">✕</button></header><nav className={styles.tabs} aria-label={`Áreas da ${profile.name}`}>{assistantWorkspacePanels(assistant).map(key => <button key={key} aria-current={tab === key ? "page" : undefined} onClick={() => { setTab(key); setError(""); setNotice(""); }}>{workspaceLabel(key, assistant)}</button>)}</nav><div className={`${styles.body} ${conversationPanel ? styles.monitorBody : ""}`}>
    {error && <p role="alert" className={styles.error}>{error}</p>}{notice && <p role="status" className={styles.notice}>{notice}</p>}
    {tab === "whatsapp-conversations" ? (assistant === "arisa" ? <ArisaWhatsAppConversations key={organizationId} organizationId={organizationId} userId={userId} /> : <p>As conversas comerciais estão disponíveis no WhatsApp da Bia.</p>) : tab === "leads" || tab === "simulations" ? <BiaCommercialPanel organizationId={organizationId} section={tab} /> : tab === "email" || tab === "agenda" ? <ArisaMailPanel key={tab} organizationId={organizationId} userId={userId} section={tab} /> : tab === "whatsapp" ? (assistant === "bia" ? <BiaWhatsAppPanel organizationId={organizationId} userId={userId} /> : <ArisaWhatsAppPanel organizationId={organizationId} />) : <>
