...
const consentText="Declaro que li, compreendi e aceito integralmente este contrato, concordando com seu conteúdo e com o plano de pagamento apresentado.";
function errorText(error:unknown){if(error instanceof Error)return error.message;if(typeof error==="string")return error;try{return JSON.stringify(error)}catch{return "Não foi possível concluir o aceite."}}
...
export default function PublicContractPage(){
 ...
 return <main className="public-proposal public-contract">
  ...
  <section className="e-sign-panel">
   {!signature.company_signed?<div className="e-sign-waiting"><small>ACEITE DA EMPRESA PENDENTE</small><h2>Aguardando emissão formal</h2><p>Solicite ao consultor o envio formal do documento.</p></div>:
   signature.customer_signed?<div className="e-sign-success"><small>ACEITE ELETRÔNICO REGISTRADO</small><h2>Contrato aceito pelas partes</h2><p>Os recebíveis foram integrados automaticamente ao fluxo financeiro.</p><dl><div><dt>Empresa</dt><dd>{signature.company_signer} · {new Date(signature.company_signed_at).toLocaleString("pt-BR")}</dd></div><div><dt>Comprador</dt><dd>{signature.customer_signer} · {new Date(signature.customer_signed_at).toLocaleString("pt-BR")}</dd></div><div><dt>Hash do documento</dt><dd>{signature.document_hash}</dd></div></dl></div>:
   <form onSubmit={accept}>
    <small>ACEITE ELETRÔNICO DO CONTRATO</small><h2>Aceitar contrato</h2>
    <p>O aceite registra timestamp, IP, navegador, hash do documento e, somente mediante autorização, localização do dispositivo.</p>
    <div className="company-signature-summary"><strong>Aceito pela Évora Urbanismo</strong><span>{signature.company_signer} · {new Date(signature.company_signed_at).toLocaleString("pt-BR")}</span><code>{signature.document_hash}</code></div>
    ...
    <label className="e-sign-check discreet-check"><input type="checkbox" checked={geoConsent} onChange={event=>setGeoConsent(event.target.checked)}/><span>Autorizo o registro da localização como evidência adicional. O aceite continua possível sem essa autorização.</span></label>
    {error&&<div className="feedback error">{error}</div>}
    <button className="primary wide" disabled={busy||!terms}>{busy?"Registrando aceite...":"Aceitar contrato"}</button>
   </form>}
