...
export function CashView({ data }: { data: ErpData }) {
  ...
  const financialOutgoing = data.entries.filter(entry => !isSettled(entry) && entry.type === "saida" && daysUntil(entry.due_date) <= period).reduce((sum, entry) => sum + Number(entry.amount), 0);
