...
export function ActivityPlanner({context}:{context:Ctx}){
 const[items,setItems]=useState<Activity[]>([]),[notifications,setNotifications]=useState<Notification[]>([]),[mode,setMode]=useState<Mode>("board"),[show,setShow]=useState(false),[message,setMessage]=useState("");
 ...
 async function load(){
  ...
  await supabase.rpc("generate_my_overdue_activity_notifications",{p_organization_id:context.organization.id});
 ...
 const overdue=items.filter(item=>item.board_status!=="concluida"&&item.due_at&&new Date(item.due_at)<now),done=items.filter(item=>item.board_status==="concluida"),unread=notifications.filter(item=>!item.read_at).length;
 ...
 return <div className="agenda-shell agenda-v65">
  ...
  <section className="agenda-card"><header><div><small>OPERAÇÃO COLABORATIVA</small><h2>Atividades da equipe</h2><p>{canAssign?"Distribua responsabilidades e acompanhe a execução.":"Acompanhe as atividades designadas a você."}</p></div><div><button className={mode==="board"?"active":""} onClick={()=>setMode("board")}>Quadro</button><button className={mode==="list"?"active":""} onClick={()=>setMode("list")}>Lista</button><button className={mode==="notifications"?"active":""} onClick={()=>setMode("notifications")}>Notificações {unread?`(${unread})`:""}</button><button className="primary" onClick={()=>setShow(true)}>+ Nova atividade</button></div></header>
   {mode==="board"?<div className="task-board">{columns.map(column=><section key={column.id}><header><strong>{column.label}</strong><span>{items.filter(item=>(item.board_status||"backlog")===column.id).length}</span></header><div>{items.filter(item=>(item.board_status||"backlog")===column.id).map(card)}</div></section>)}</div>:mode==="list"?<div className="agenda-list">{items.map(card)}{!items.length&&<p className="empty-state">Nenhuma atividade cadastrada.</p>}</div>:<div className="activity-notifications"><header><div><strong>Central de notificações</strong><span>Designações, alterações, progresso e atrasos.</span></div>{unread>0&&<button onClick={markAllRead}>Marcar todas como lidas</button>}</header>{notifications.map(notification=><article key={notification.id} className={notification.read_at?"read":"unread"} onClick={()=>!notification.read_at&&markRead(notification.id)}><i data-type={notification.notification_type}>●</i><div><strong>{notification.title}</strong><p>{notification.message}</p><small>{notification.actor_user_id?`${memberName(notification.actor_user_id)} · `:""}{new Date(notification.created_at).toLocaleString("pt-BR")}</small></div>{!notification.read_at&&<b>Nova</b>}</article>)}{!notifications.length&&<p className="empty-state">Nenhuma notificação registrada.</p>}</div>}
  </section>
