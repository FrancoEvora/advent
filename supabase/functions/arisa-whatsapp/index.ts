...
import { arisaOutbound } from "../_shared/arisa-whatsapp-outbound.ts";
...
export async function handleWhatsApp(request:Request):Promise<Response>{
  ...
  try{
    ...
    if(!isObject(body)||typeof body.organizationId!=="string"||!UUID.test(body.organizationId)||typeof body.action!=="string"||!["outbound_preview","outbound_send","outbound_status","status","templates","send","get","reconcile","list","configure","attention","save_recipient","authorize","deny"].includes(body.action))throw new ManagerError("WHATSAPP_INVALID",400);
    ...
    if(body.action.startsWith("outbound_")){const result=await arisaOutbound(admin,body.organizationId,actor,body.action.slice(9),args);return json({ok:true,data:result});}
