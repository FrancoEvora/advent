...
export async function analyzeWhatsAppAttention(history: Obj[], config: Obj, request: typeof fetch=fetch) {
  ...
  const response=await request("https://api.openai.com/v1/responses",{
    ...
    body:JSON.stringify({model:config.agent_model,store:false,max_output_tokens:1400,instructions:ATTENTION_INSTRUCTIONS,
      ...
      input:[{role:"user",content:JSON.stringify({latest_inbound:history.filter(row=>row.direction==="inbound").at(-1)?.content||"",message_time:history.filter(row=>row.direction==="inbound").at(-1)?.occurred_at||null,timezone:"America/Sao_Paulo",history:history.slice(-20).map(row=>({direction:row.direction,occurred_at:row.occurred_at||null,content:String(row.content||"").slice(0,6000)}))})}],
