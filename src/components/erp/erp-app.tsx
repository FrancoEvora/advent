...
export default function ErpApp() {
  ...
  const refresh = useCallback(async (currentSession = session) => {
    ...
    try {
      ...
      const [org, profile, entries, centers, categories, accounts, contacts, projects, members, profiles, invitations, approvals, audit, settings] = await Promise.all([
        ...
        supabase.from("audit_logs").select("*").eq("organization_id", orgId).order("created_at", { ascending: false }).limit(200),
  ...
  const mutate = async (operation: () => Promise<void>, success: string) => {
    ...
    catch (error) { setNotice(error instanceof Error ? error.message : "A operação não foi concluída."); }
    finally { setLoading(false); }
  };
  ...
  const permittedNav = nav.filter((item) => !["usuarios", "auditoria", "configuracoes"].includes(item.id) || canAdmin(data.membership.role));
  const title = nav.find((item) => item.id === view)?.label ?? "Évora Gestão";

  return <div className="erp-shell">
    <aside className="erp-sidebar">
      <Brand />
      <div className="org-pill"><span>AMBIENTE</span><strong>{data.organization.trade_name || data.organization.name}</strong></div>
      {[...new Set(permittedNav.map((item) => item.group))].map((group) => <div className="nav-group" key={group}><small>{group}</small>{permittedNav.filter((item) => item.group === group).map((item) => <button key={item.id} className={view === item.id ? "active" : ""} onClick={() => setView(item.id)}><b>{item.icon}</b><span>{item.label}</span></button>)}</div>)}
      <div className="sidebar-user"><span>{initials(userName)}</span><div><strong>{userName}</strong><small>{roleLabels[data.membership.role]}</small></div></div>
    </aside>
    <main className="erp-main">
      <header className="erp-header"><div><small>ÉVORA URBANISMO · ERP</small><h1>{title}</h1></div><div className="header-actions"><button className="icon-button" onClick={() => refresh()} title="Atualizar">↻</button><button className="primary" onClick={() => setView("financeiro")}>+ Novo lançamento</button></div></header>
      {notice && <button className="notice" onClick={() => setNotice("")}>{notice}<span>×</span></button>}
      {loading && <div className="progress"><i /></div>}
      <section className="erp-content">
        {view === "dashboard" && <DashboardView data={data} go={setView} />}
        {view === "caixa" && <CashView data={data} />}
        {view === "financeiro" && <FinanceView data={data} mutate={mutate} />}
        {view === "centros" && <MastersView mode="centros" data={data} mutate={mutate} />}
        {view === "cadastros" && <MastersView mode="cadastros" data={data} mutate={mutate} />}
        {view === "projetos" && <MastersView mode="projetos" data={data} mutate={mutate} />}
        {["aprovacoes", "usuarios", "relatorios", "auditoria", "configuracoes"].includes(view) && <AdminView mode={view} data={data} mutate={mutate} />}
      </section>
    </main>
    <nav className="mobile-nav">{permittedNav.slice(0, 5).map((item) => <button key={item.id} className={view === item.id ? "active" : ""} onClick={() => setView(item.id)}><b>{item.icon}</b><small>{item.label.split(" ")[0]}</small></button>)}</nav>
  </div>;
}
...
function Auth() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const form = new FormData(event.currentTarget); const supabase = getSupabase();
    if (!supabase) { setMessage("Configuração do Supabase não encontrada."); setBusy(false); return; }
