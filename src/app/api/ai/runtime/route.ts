...
function optionalBoolean(value: unknown, field: string): boolean | null {
  ...
  if (typeof value !== "boolean") throw new ApiError(`${field} inválido.`, 400, "INVALID_ENABLED_STATE");
  return value;
}
function optionalModel(value: unknown, code: string): string | null {
  if (value === undefined || value === null) return null;
  if (typeof value !== "string" || !MODEL.test(value)) throw new ApiError("Modelo inválido.", 400, code);
  return value;
}
function optionalReasoning(value: unknown, code: string): string | null {
  if (value === undefined || value === null) return null;
  if (typeof value !== "string" || !REASONING.has(value)) throw new ApiError("Raciocínio inválido.", 400, code);
  return value;
}
function optionalApiKey(value: unknown): string | null {
  if (value === undefined || value === null || value === "") return null;
  if (typeof value !== "string" || value !== value.trim() || value.length < 32 || value.length > 512 || /\s/.test(value)) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (result.error) {
      ...
      throw new ApiError(forbidden ? "Seu perfil não pode gerenciar a Bia." : "Status da Bia indisponível.", forbidden ? 403 : 503, forbidden ? "AI_RUNTIME_PERMISSION_REQUIRED" : "AI_RUNTIME_STATUS_FAILED");
    }
    ...
    return NextResponse.json({ ok: false, error: known?.code || "AI_RUNTIME_STATUS_UNAVAILABLE" }, { status: known?.status || 503, headers: HEADERS });
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    // O atendimento IA-first é síncrono na própria Edge Function da Bia.
    // Não prepara nem depende do worker/fila do pipeline legado ao ativar a Bia.
    ...
    if (result.error) {
      const forbidden = result.error.code === "42501";
      throw new ApiError(forbidden ? "Seu perfil não pode gerenciar a Bia." : "Configuração da Bia não pôde ser salva.", forbidden ? 403 : 400, forbidden ? "AI_RUNTIME_PERMISSION_REQUIRED" : "AI_RUNTIME_CONFIG_FAILED");
    }
    return NextResponse.json({ ok: true, runtime: sanitizedStatus(result.data) }, { status: 200, headers: HEADERS });
  } catch (error) {
    const known = error instanceof ApiError ? error : null;
    return NextResponse.json({ ok: false, error: known?.code || "AI_RUNTIME_CONFIG_UNAVAILABLE" }, { status: known?.status || 503, headers: HEADERS });
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (result.error) {
      const forbidden = result.error.code === "42501";
      throw new ApiError(forbidden ? "Seu perfil não pode gerenciar a Bia." : "A chave da Bia não pôde ser revogada.", forbidden ? 403 : 503, forbidden ? "AI_RUNTIME_PERMISSION_REQUIRED" : "AI_RUNTIME_REVOKE_FAILED");
    }
    return NextResponse.json({ ok: true, runtime: sanitizedStatus(result.data) }, { status: 200, headers: HEADERS });
  } catch (error) {
    const known = error instanceof ApiError ? error : null;
    return NextResponse.json({ ok: false, error: known?.code || "AI_RUNTIME_REVOKE_UNAVAILABLE" }, { status: known?.status || 503, headers: HEADERS });
  }
}
