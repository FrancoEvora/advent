...
import { processWhatsAppNotices } from "../_shared/arisa-whatsapp-attention.ts";
...
Deno.serve(async request => {
  ...
  try { return json({ok:true,...await processWhatsAppNotices(admin)}); }
  catch { return json({ok:false,error:"WHATSAPP_NOTICE_WORKER_UNAVAILABLE"},503); }
});
