...
export function ProcurementView({data,mutate,can:permissionCheck,focus}:{data:ErpData;mutate:(operation:()=>Promise<void>,success:string)=>Promise<void>;can?:(key:string)=>boolean;focus?:ActivityDeepLinkTarget|null}){
 const can=permissionCheck??((key:string)=>data.membership.role==="admin"||data.membership.permissions?.[key]===true);
 const canViewFuel=can("fuel.view");
 const initialTab=focus?.sourceType==="fuel_requests"&&canViewFuel?"combustiveis":"solicitacoes";
 const[tab,setTab]=useState<"solicitacoes"|"combustiveis">(initialTab),[create,setCreate]=useState(false),[documents,setDocuments]=useState<PurchaseRequest|null>(null),[selected,setSelected]=useState<PurchaseDecision|null>(null);
