...
export async function analyzeWhatsAppAttention(history: Obj[], config: Obj, request: typeof fetch=fetch, followUp: Obj | null = null) {
  ...
  const response=await request("https://api.openai.com/v1/responses",{
    ...
    body:JSON.stringify({model:config.agent_model,store:false,max_output_tokens:1400,instructions:ATTENTION_INSTRUCTIONS+"\nSe outreach estiver presente, a Arisa iniciou o contato para tratar a pergunta preparada para este destinatário. Use esse assunto para entender a resposta: um aceite para conversar não é um novo atendimento comercial nem exige notificação. Se a pessoa já responder concretamente ao assunto, resuma o retorno e sinalize needs_notification=true. Não invente nomes nem divulgue dados privados.",
      ...(/^(gpt-5|o\d)/.test(config.agent_model)?{reasoning:{effort:"low"}}:{}),
      input:[{role:"user",content:JSON.stringify({outreach:followUp,latest_inbound:history.filter(row=>row.direction==="inbound").at(-1)?.content||"",message_time:history.filter(row=>row.direction==="inbound").at(-1)?.occurred_at||null,timezone:"America/Sao_Paulo",history:history.slice(-20).map(row=>({direction:row.direction,occurred_at:row.occurred_at||null,content:String(row.content||"").slice(0,6000)}))})}],
