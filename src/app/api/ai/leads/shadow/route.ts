...
class ApiError extends Error {
  ...
  constructor(message: string, status = 400, code = "AI_SHADOW_REQUEST_FAILED") {
    super(message);
    this.name = "ApiError";
    this.status = status;
    this.code = code;
...
function serviceConfig() {
  ...
  if (!url || !key) throw new ApiError("Supabase de integração indisponível.", 503, "SUPABASE_SERVICE_UNAVAILABLE");
  return { url, key };
}
...
async function authContext(request: NextRequest, organizationId: string): Promise<AuthContext> {
  if (!UUID.test(organizationId)) throw new ApiError("Organização inválida.", 400, "INVALID_ORGANIZATION");
  ...
  if (permission.error || permission.data !== true) {
    throw new ApiError("Seu perfil não pode consultar a Vitória.", 403, "COPILOT_PERMISSION_REQUIRED");
  }
...
function normalizeRecordIds(value: unknown): string[] {
  ...
  if (unique.length !== value.length || unique.some((id) => !UUID.test(id))) {
    throw new ApiError("Lista de oportunidades inválida.", 400, "INVALID_RECORD_LIST");
  }
  return unique;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isObj(raw)) throw new ApiError("Dados inválidos.", 400, "INVALID_REQUEST");
    ...
    const records = await auth.service
      ...
      .in("id", crmRecordIds);
    if (records.error) {
      throw new ApiError("Não foi possível validar as oportunidades.", 503, "CRM_RECORD_VALIDATION_FAILED");
    }
    const allowedIds = new Set((records.data || []).map((row) => String(row.id)));
    if (allowedIds.size !== crmRecordIds.length) {
      throw new ApiError("Uma ou mais oportunidades não pertencem à organização.", 403, "RECORD_SCOPE_REJECTED");
    }

    const conversations = await auth.service
    ...
    if (conversations.error) {
      throw new ApiError("A leitura da Vitória está indisponível.", 503, "AI_CONVERSATION_READ_FAILED");
    }
    ...
    const messages = conversationIds.length
      ? await auth.service
          .from("crm_messages")
          .select("conversation_id,crm_record_id,content,delivery_status,metadata,occurred_at")
          .eq("organization_id", auth.organizationId)
          .eq("actor_type", "ai")
      ...
      : { data: [], error: null };
    if (messages.error) {
      throw new ApiError("Os rascunhos da Vitória estão indisponíveis.", 503, "AI_MESSAGE_READ_FAILED");
    }

    const latestMessage = new Map<string, Obj>();
    for (const row of messages.data || []) {
      const recordId = String(row.crm_record_id || "");
      if (recordId && !latestMessage.has(recordId)) latestMessage.set(recordId, row as Obj);
    }

    const response: ShadowRow[] = conversationRows.map((row) => {
      const recordId = String(row.crm_record_id);
      ...
      return {
        ...
        status: String(row.status || "shadow"),
        draft: message && message.delivery_status === "draft" ? stringOrNull(message.content) : null,
        qualityScore: metadata ? numberOrNull(metadata.quality_score) : null,
        supervisorDecision: metadata ? stringOrNull(metadata.supervisor_decision) : null,
        updatedAt:
          stringOrNull(message?.occurred_at) ||
          stringOrNull(row.last_message_at) ||
