...
function Conversation({ userId, organizationId, organizationName, initialThreadId, initialPanel }: { userId: string; organizationId: string; organizationName: string; initialThreadId: string | null; initialPanel: WorkspacePanel | null }) {
  ...
  return <main id="conteudo-principal" className="public-agent-page bia-whatsapp arisa-chat">{panel && <ArisaWorkspace organizationId={organizationId} userId={userId} initialPanel={panel} onPanelChange={openPanel} onClose={() => openPanel(null)} />}<div className="public-agent-shell"><section className="public-agent-chat-card"><Header organizationId={organizationId} menu={() => setMenu(previous => !previous)} voice={voice} recording={recording} />
    {menu && <AssistantConversationMenu organization={organizationName} conversations={threads} selectedId={threadId} busy={busy || recording} onClose={() => setMenu(false)} onNew={() => switchThread(null)} onSelect={switchThread}>{workspacePanels.map(key => <button key={key} disabled={busy || recording} onClick={() => openPanel(key)}>{workspaceLabels[key]} da Arisa</button>)}<Link href="/">Abrir plataforma</Link><Link href="/?view=arisa">Fila de documentos</Link><Link href="/agenda">Agenda da plataforma</Link><button disabled={busy || recording} onClick={() => void client().auth.signOut()}>Sair da conta</button></AssistantConversationMenu>}
    ...
    <details className="bia-chat-privacy"><summary>Gestão com IA · Acesso privado</summary><div><p>Conversas e documentos ficam vinculados à sua conta e organização. Os pedidos são executados com sua alçada administrativa e registrados na auditoria.</p><p>Não envie senhas. A Arisa registra e programa operações financeiras; não realiza transferências bancárias. Você pode ouvir sua gravação antes de enviar.</p><Link href="/privacidade">Política de Privacidade</Link><br /><Link href="/">Évora Gestão · Versão 6.29</Link></div></details>
  </section></div></main>;
}
