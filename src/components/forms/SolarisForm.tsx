      ...
      <div className={styles.grid}>
        ...
        <section className={styles.panel} aria-label="Cadastro de interesse no Solaris">
          ...
          <form onSubmit={next} noValidate>
            ...
            {error&&<p className={styles.error} role="alert">{error}</p>}{page<3&&<div className={styles.actions}>{page>1&&<button type="button" className={styles.back} disabled={busy} onClick={()=>{setError("");setPage(page-1)}}>‹ Voltar</button>}<button type="submit" className={styles.continue} disabled={busy}>{busy?"Registrando…":page===2?"Registrar meu interesse →":"Continuar →"}</button></div>}
