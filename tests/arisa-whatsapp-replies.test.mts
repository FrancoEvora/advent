...
test("queue fences changed context before any WhatsApp send",async()=>{
  ...
  const admin={rpc:async(name,args)=>{calls.push(name);if(name==="get_crm_ai_runtime_credentials")return {data:config};if(args.p_action==="claim"){if(claimed)return {data:{}};claimed=true;return {data:{id:"11111111-1111-4111-8111-111111111111",lease:"lease",organization_id:"org",history:[{direction:"inbound",content:"Oi"}]}};}if(args.p_action==="send")return {data:{proceed:false}};throw Error("Unexpected send");}};
  const result=await processWhatsAppReplies(admin as never,{limit:1,request:(async()=>Response.json(completed())) as typeof fetch});
...
test("AI failure retries generation but never creates a send operation",async()=>{
  ...
  assert.equal(failure.sending,false);assert.equal(failure.error,"WHATSAPP_REPLY_AI_UNAVAILABLE");
});
