...
type Message = { id: string; content: string; direction: string; delivery_status: string | null; occurred_at: string };
...
export default function BiaInbox() {
  ...
  useEffect(() => {
    ...
    return () => { alive=false; invalidate();subscription.subscription.unsubscribe();document.documentElement.classList.remove('public-agent-active');document.body.classList.remove('public-agent-active'); };
  }, [invalidate]);
