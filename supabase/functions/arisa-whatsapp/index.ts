...
export async function handleWhatsApp(request:Request):Promise<Response>{
  ...
  try{
    ...
    if(!isObject(body)||typeof body.organizationId!=="string"||!UUID.test(body.organizationId)||typeof body.action!=="string"||!["status","templates","send","get","reconcile","list","configure","attention","save_recipient","authorize","deny"].includes(body.action))throw new ManagerError("WHATSAPP_INVALID",400);
