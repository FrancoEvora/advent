...
export function VoiceBar({ voice, assistantName = "Arisa" }: { voice: ArisaVoice; assistantName?: string }) {
  const { state } = voice;
  if (!state.enabled && !state.error) return null;
  const active = !!state.messageId;
  return <section className={styles.bar} aria-label={`Controles da voz da ${assistantName}`}>
    <div><strong>{state.phase === "paused" ? "Fala pausada" : state.phase === "speaking" ? `${assistantName} está falando` : state.phase === "loading" ? "Preparando voz…" : "Modo fala ligado"}</strong><small>Voz gerada por IA · Português brasileiro{active && state.part > 0 ? ` · Trecho ${state.part} de ${state.total}` : ""}</small></div>
...
export function VoiceMessage({ message, voice, disabled, assistantName = "Arisa" }: { message: Message; voice: ArisaVoice; disabled?: boolean; assistantName?: string }) {
