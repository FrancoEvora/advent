-- Server-only ingestion; the public form cannot read CRM or change its organization.
create table private.crm_public_forms (
 ...
 lead_source_id uuid not null references public.crm_lead_sources(id),
 landing_page text not null,
 active boolean not null default true
);
create table private.crm_public_form_submissions (
 id uuid primary key,
 form_slug text not null references private.crm_public_forms(slug),
 organization_id uuid not null references public.organizations(id),
 crm_record_id uuid not null references public.crm_records(id),
 person_name text not null,
 phone text not null,
 purpose text not null check(purpose in ('investir','morar')),
 budget text not null check(budget in ('300_500','acima_500')),
 consent_version text not null,
 attribution jsonb not null default '{}'::jsonb,
 created_at timestamptz not null default now(),
 original_created_at timestamptz
);
create index crm_public_form_submissions_record_idx on private.crm_public_form_submissions(crm_record_id);
create table private.crm_public_form_limits (
 key text primary key,
 count integer not null,
 expires_at timestamptz not null
);
create index crm_public_form_limits_expiry_idx on private.crm_public_form_limits(expires_at);
alter table private.crm_public_forms enable row level security;
alter table private.crm_public_form_submissions enable row level security;
alter table private.crm_public_form_limits enable row level security;
revoke all on private.crm_public_forms,private.crm_public_form_submissions,private.crm_public_form_limits from public,anon,authenticated;
grant select on private.crm_public_forms to service_role;
grant select,insert on private.crm_public_form_submissions to service_role;
grant select,insert,update,delete on private.crm_public_form_limits to service_role;
grant usage on schema private to service_role;

create function public.submit_solaris_public_form(p_submission jsonb,p_fingerprint text)
...
begin
 ...
 if person is null or length(person) not between 3 and 120 or phone_number is null or phone_number !~ '^\+55[1-9][0-9][2-9][0-9]{7,8}$'
    ...
    or investment is null or investment not in ('300_500','acima_500') or jsonb_typeof(attrs)<>'object' or pg_column_size(attrs)>4096 then raise exception 'FORM_INVALID'; end if;
 ...
 if found then
   if previous.person_name<>person or previous.phone<>phone_number or previous.purpose<>objective or previous.budget<>investment then raise exception 'FORM_ID_CONFLICT'; end if;
 ...
 detail:='Formulário Futura Casa / Solaris. Registro '||upper(left(request_id::text,8))||'. Objetivo: '||objective||'. Faixa: '||case when investment='300_500' then 'entre R$ 300 mil e R$ 500 mil' else 'acima de R$ 500 mil' end||'. Autorização de contato por WhatsApp sobre o Solaris: aceita ('||'solaris-whatsapp-v1'||').';
