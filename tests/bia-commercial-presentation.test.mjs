...
const gateway = fs.readFileSync(
  "supabase/functions/enterprise-bia-agent-gateway/index.ts",
  "utf8",
);
...
test("gateway entrega saída para todas as ferramentas e usa cálculo canônico", () => {
  ...
  assert.match(gateway, /for \(const call of calls\)/u);
  assert.match(gateway, /type: "function_call_output"/u);
  assert.match(gateway, /call_id: call\.callId/u);
  ...
  assert.match(gateway, /unitCode: str\(args\.unit_code\)/u);
  assert.match(gateway, /areaMin: num\(args\.area_min\)/u);
  assert.match(gateway, /budgetMax: num\(args\.budget_max\)/u);
});
...
test("respostas comerciais são persistidas como texto limpo", () => {
  ...
  assert.match(gateway, /reply: cleanReply\(reply\)/u);
});
