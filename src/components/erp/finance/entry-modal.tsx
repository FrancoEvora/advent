...
function safeName(value: string) {
  return value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-zA-Z0-9._-]+/g, "-").slice(-100);
}
...
export function EntryModal({ data, revenueCenters, entry, close, mutate }: { data: ErpData; revenueCenters: RevenueCenter[]; entry: FinancialEntry | null; close: () => void; mutate: (operation: () => Promise<void>, success: string) => Promise<void> }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const maxBytes = Number(data.settings.document_max_size_mb || 20) * 1024 * 1024;
    const oversized = files.find(file => file.size > maxBytes);
    if (oversized) {
      setError(`O arquivo “${oversized.name}” supera o limite de ${data.settings.document_max_size_mb || 20} MB.`);
      return;
    }
    ...
    await mutate(async () => {
      ...
      if (entry) {
        const result = await supabase.from("financial_entries").update(payload).eq("id", entry.id);
        if (result.error) throw new Error(result.error.message);
      } else {
        const result = await supabase.from("financial_entries").insert(payload).select().single();
        if (result.error) throw new Error(result.error.message);
      ...
      if (entryId && files.length) {
        const documentType = String(form.get("attachment_type") || "nota_fiscal");
        for (const file of files) {
          const path = `${data.organization.id}/financial_entry/${entryId}/${crypto.randomUUID()}-${safeName(file.name)}`;
          const upload = await supabase.storage.from("erp-documents").upload(path, file, { contentType: file.type || undefined, upsert: false });
          if (upload.error) throw new Error(`Falha ao enviar ${file.name}: ${upload.error.message}`);
          const metadata = await supabase.from("document_attachments").insert({ organization_id: data.organization.id, entity_type: "financial_entry", entity_id: entryId, document_type: documentType, file_name: file.name, storage_path: path, mime_type: file.type || null, size_bytes: file.size, uploaded_by: data.session.user.id });
          if (metadata.error) {
            await supabase.storage.from("erp-documents").remove([path]);
            throw new Error(`Falha ao vincular ${file.name}: ${metadata.error.message}`);
          }
        }
      }
    }, entry ? "Lançamento e documentos atualizados." : requiresApproval ? "Lançamento enviado para aprovação com os documentos anexados." : "Lançamento salvo com os documentos anexados.");
    close();
  }
  ...
  return <>
    ...
    {documentsOpen && entry && <EntityDocumentModal data={data} mutate={mutate} entityType="financial_entry" entityId={entry.id} close={() => setDocumentsOpen(false)} />}
  </>;
}