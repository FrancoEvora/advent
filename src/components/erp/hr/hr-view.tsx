...
export function HrView({ data, mutate, focus }: { data: ErpData; mutate: (operation: () => Promise<void>, success: string) => Promise<void>; focus?: ActivityDeepLinkTarget | null }) {
  const [tab, setTab] = useState<"colaboradores" | "folhas" | "eventos">(
    focus?.sourceType === "hr_events" ? "eventos" : "colaboradores",
  );
