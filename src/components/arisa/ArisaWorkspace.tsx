...
import { assistantWorkspacePanels, workspaceLabels } from "./workspace-navigation";
...
export default function ArisaWorkspace({ organizationId, userId, initialPanel: tab, onPanelChange: setTab, onClose, assistant = "arisa" }: { assistant?: AssistantId; organizationId: string; userId: string; initialPanel: WorkspacePanel; onPanelChange: (panel: WorkspacePanel) => void; onClose: () => void }) {
  ...
  return <dialog ref={dialog} className={`${styles.dialog} ${tab === "whatsapp-conversations" ? styles.monitorDialog : ""}`} onCancel={onClose} aria-labelledby="arisa-workspace-title"><header className={styles.header}><div><small>{profile.name.toUpperCase()} · ÉVORA URBANISMO</small><h2 id="arisa-workspace-title">Canais e conhecimento</h2></div><button onClick={onClose} aria-label="Fechar e voltar à conversa">✕</button></header><nav className={styles.tabs} aria-label={`Áreas da ${profile.name}`}>{assistantWorkspacePanels(assistant).map(key => <button key={key} aria-current={tab === key ? "page" : undefined} onClick={() => { setTab(key); setError(""); setNotice(""); }}>{workspaceLabels[key]}</button>)}</nav><div className={`${styles.body} ${tab === "whatsapp-conversations" ? styles.monitorBody : ""}`}>
