...
import { workspaceLabel, workspacePanels } from "./workspace-navigation";
...
export default function ArisaWorkspace({ organizationId, userId, initialPanel: tab, onPanelChange: setTab, onClose, assistant = "arisa" }: { assistant?: AssistantId; organizationId: string; userId: string; initialPanel: WorkspacePanel; onPanelChange: (panel: WorkspacePanel) => void; onClose: () => void }) {
  ...
  return <dialog ref={dialog} className={styles.dialog} onCancel={onClose} aria-labelledby="arisa-workspace-title"><header className={styles.header}><div><small>{profile.name.toUpperCase()} · ÉVORA URBANISMO</small><h2 id="arisa-workspace-title">{assistant === "bia" && tab === "whatsapp" ? "Conversas da Bia" : "Canais e conhecimento"}</h2></div><button onClick={onClose} aria-label="Fechar e voltar à conversa">✕</button></header><nav className={styles.tabs} aria-label={`Áreas da ${profile.name}`}>{workspacePanels.filter(key => assistant === "bia" || !["leads", "simulations"].includes(key)).map(key => <button key={key} aria-current={tab === key ? "page" : undefined} onClick={() => { setTab(key); setError(""); setNotice(""); }}>{workspaceLabel(key, assistant)}</button>)}</nav><div className={styles.body}>
    {error && <p role="alert" className={styles.error}>{error}</p>}{notice && <p role="status" className={styles.notice}>{notice}</p>}
    {tab === "leads" || tab === "simulations" ? <BiaCommercialPanel organizationId={organizationId} section={tab} /> : tab === "email" || tab === "agenda" ? <ArisaMailPanel key={tab} organizationId={organizationId} userId={userId} section={tab} /> : tab === "whatsapp" ? (assistant === "bia" ? <BiaWhatsAppPanel organizationId={organizationId} userId={userId} /> : <ArisaWhatsAppPanel organizationId={organizationId} />) : <>
