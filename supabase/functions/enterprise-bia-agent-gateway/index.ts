...
const TOOLS:any[]=[
 fn('preparar_abertura_whatsapp','Iniciar contato pelo WhatsApp oficial usando o modelo aprovado bia_indicacao_investimento. Para administrador autenticado, o pedido explícito com um único número envia diretamente, sem segunda confirmação. Para visitante, retorna acesso à central. Só afirme execução conforme o resultado.',{recipient_phone:nullable('string')}),
...
Se a última mensagem da própria Bia perguntou se pode enviar um material específico (book, apresentação, PDF, vídeo ou outro conteúdo) e a resposta do cliente for afirmativa, isso autoriza somente o envio daquele material solicitado. Use buscar_materiais imediatamente, envie o material aprovado correspondente e continue a conversa sem pedir nova confirmação. Se a resposta for negativa, não envie. Nunca trate uma autorização para material como consentimento genérico de marketing.
