...
const core = fs.readFileSync("supabase/functions/enterprise-bia-agent-gateway/core.ts", "utf8");
...
test("condições comerciais são preparadas antes da resposta complementar", () => {
  for (const term of ["Condições comerciais:", "• Entrada:", "• Parcelas:", "• Juros:", "• Correção:", "• Balões:"]) assert.ok(route.includes(term));
...
test("gateway entrega saída para todas as ferramentas e usa cálculo canônico", () => {
  assert.match(core, /function toolCalls\(/u);
  ...
  assert.match(core, /unitCode:unitCode\(code\)/u);
  assert.match(core, /areaMin:positive\(args\.area_min\)/u);
  assert.match(core, /budgetMax:positive\(args\.budget_max\)/u);
});
test("respostas comerciais são persistidas como texto limpo", () => {
  assert.match(core, /function cleanReply\(/u);
  assert.match(gateway, /sem Markdown, sem asteriscos/u);
  assert.match(gateway, /reply\s*:\s*cleanReply\(reply\)/u);
  assert.match(gateway, /finish_bia_turn_v1/u);
});
