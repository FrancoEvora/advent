begin;
create or replace function public.set_vitoria_knowledge_source_status(
...
begin
  if p_status not in ('pending','processing','completed','failed') then
    raise exception 'VITORIA_KNOWLEDGE_STATUS_INVALID';
  end if;
  update crm_private.vitoria_knowledge_sources
  set vector_file_status=p_status,updated_at=now()
