...
export default function ArisaWhatsAppAttention({organizationId}:{organizationId:string}){
 ...
 useEffect(()=>{let alive=true;workspaceCall("arisa-whatsapp",{action:"attention",organizationId}).then(value=>{if(alive){setError("");setData(value);setPhones(Object.fromEntries((value.recipients||[]).map((r:Recipient)=>[r.user_id,r.phone||""])));}}).catch(reason=>{if(alive)setError(errorText(reason));});return()=>{alive=false};},[organizationId,revision]);
 ...
 return <section className={styles.card} aria-labelledby="arisa-attention-title">
  ...
  <p>Clientes e fornecedores podem consultar apenas o próprio financeiro após confirmar nome, CPF/CNPJ e e-mail, pelo WhatsApp cadastrado. Propostas de negociação aguardam aprovação. As demais informações internas e sensíveis permanecem protegidas. Uma aprovação permite enviar somente o texto escrito abaixo ao contato indicado; ela não libera outros dados.</p>
