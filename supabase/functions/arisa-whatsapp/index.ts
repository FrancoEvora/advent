...
export async function handleWhatsApp(request:Request):Promise<Response>{
  ...
  try{
    ...
    if(!isObject(body)||typeof body.organizationId!=="string"||!UUID.test(body.organizationId)||typeof body.action!=="string"||!["status","templates","send","get","reconcile"].includes(body.action))throw new ManagerError("WHATSAPP_INVALID",400);
    ...
    const result=await runWhatsAppTool(admin,body.organizationId,actor,body.action,args,context);return json({ok:true,...result});
