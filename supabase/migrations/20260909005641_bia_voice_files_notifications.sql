create table crm_private.bia_customer_files (
  id uuid primary key default gen_random_uuid(), session_id uuid not null references crm_private.public_agent_sessions(id) on delete cascade,
  client_id uuid not null, name text not null, mime text not null, size_bytes integer not null check(size_bytes between 1 and 8388608),
  sha256 text not null check(sha256 ~ '^[a-f0-9]{64}$'), storage_path text not null unique,
  ready boolean not null default false, message_id bigint references crm_private.public_agent_messages(id) on delete set null,
  created_at timestamptz not null default now(), unique(session_id,client_id)
);
create table crm_private.bia_customer_notice_reads (
  message_id bigint primary key references crm_private.public_agent_messages(id) on delete cascade,
  read_at timestamptz not null default now()
);
create table crm_private.bia_customer_speech_usage (
  organization_id uuid not null references public.organizations(id) on delete cascade,
  fingerprint_hash text not null, day date not null default current_date, characters integer not null default 0,
  requests integer not null default 0, primary key(organization_id,fingerprint_hash,day)
);
alter table crm_private.bia_customer_files enable row level security;
alter table crm_private.bia_customer_notice_reads enable row level security;
alter table crm_private.bia_customer_speech_usage enable row level security;
revoke all on crm_private.bia_customer_files,crm_private.bia_customer_notice_reads,crm_private.bia_customer_speech_usage from public,anon,authenticated;
insert into storage.buckets(id,name,public,file_size_limit,allowed_mime_types)
values('bia-customer-files','bia-customer-files',false,8388608,array['application/pdf','image/png','image/jpeg','image/webp','text/plain','text/csv','application/xml','application/x-ofx']);

create function crm_private.bia_customer_session(p_slug text,p_token text,p_fingerprint text,p_conversation_id uuid default null)
returns crm_private.public_agent_sessions language plpgsql security definer set search_path='' as $$
...
begin
  ...
  where e.slug=lower(trim(p_slug)) and e.active and x.session_token_hash=p_token and x.fingerprint_hash=p_fingerprint;
  if not found or s.status in ('closed','blocked') or s.expires_at<=now() then raise exception 'PUBLIC_AGENT_SESSION_INACTIVE'; end if;
  if p_conversation_id is not null and s.id<>p_conversation_id then raise exception 'PUBLIC_AGENT_CONVERSATION_CHANGED'; end if;
  if exists(select 1 from crm_private.bia_whatsapp_threads w where w.session_id=s.id) then raise exception 'PUBLIC_AGENT_SESSION_INACTIVE'; end if;
  return s;
end $$;

create function public.bia_customer_tools_v1(p_slug text,p_session_token_hash text,p_fingerprint_hash text,p_conversation_id uuid,p_operation text,p_args jsonb default '{}')
...
begin
  ...
  if p_operation='files_prepare' then
    ...
    select * into reply from crm_private.public_agent_messages m where m.session_id=s.id and m.direction='assistant'
      and (m.id::text=p_args->>'messageId' or 'assistant-'||(m.metadata->>'bia_request_id')=p_args->>'messageId') order by m.id desc limit 1;
...
create function public.finish_bia_turn_with_files_v1(p_slug text,p_session_token_hash text,p_fingerprint_hash text,p_client_request_id uuid,p_lease_token uuid,p_payload jsonb,p_response jsonb)
returns jsonb language plpgsql security definer set search_path='' as $$
...
begin
  ...
  select id into a from crm_private.public_agent_messages where session_id=s.id and direction='assistant' and metadata->>'bia_request_id'=p_client_request_id::text order by id desc limit 1;
...
revoke all on function crm_private.bia_customer_session(text,text,text,uuid) from public,anon,authenticated;
revoke all on function public.bia_customer_tools_v1(text,text,text,uuid,text,jsonb) from public,anon,authenticated;
revoke all on function public.finish_bia_turn_with_files_v1(text,text,text,uuid,uuid,jsonb,jsonb) from public,anon,authenticated;
grant execute on function crm_private.bia_customer_session(text,text,text,uuid) to service_role;
grant execute on function public.bia_customer_tools_v1(text,text,text,uuid,text,jsonb) to service_role;
grant execute on function public.finish_bia_turn_with_files_v1(text,text,text,uuid,uuid,jsonb,jsonb) to service_role;
